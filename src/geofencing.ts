  ...
  for (const fence of geofences) {
    if (fence.type === "circle") {
      if (
        ...
        fence.centerLon != null &&
        fence.radiusM != null
      ) {
        if (isPointInCircle(lat, lon, fence.centerLat, fence.centerLon, fence.radiusM)) {
          return { inside: true, geofenceId: fence.id };
        }
      }
    } else if (fence.type === "polygon") {
      if (fence.polygon && Array.isArray(fence.polygon)) {
        if (isPointInPolygon(lat, lon, fence.polygon as Array<[number, number]>)) {
          return { inside: true, geofenceId: fence.id };
        }
      }
    }
